...
- **C2** ~~`polyfit.c:280–296`: condition/rank warning jen na prvním okně (`i == offset`). Horší okna později se neohlásí.~~ **FIXED v5.11.19** — per-window aggregation: tracks `worst_cond` and `rank_deficient_count`, reports once at end (same style as B7 fallback report).
